...
### Responsibility 3: Generate a detailed selector + test coverage summary Markdown
After completing Responsibilities 1 and 2, the subagent must create (or overwrite) a Markdown file:

- `QA_AUTOMATION_RUN_SUMMARY.md`

This file must be detailed and structured so that a developer can understand what was instrumented and how the automation works, without opening the code.

Minimum required sections of the summary file:

- **Overview**
  - Store/theme context (base URL if configured)
  - Whether the project uses cart page vs cart drawer (and how detected)
- **Sections instrumented (by template)**
  - For each template (`index`, `collection`, `product`, `cart`):
    - List each section `type` used and the corresponding file(s) updated (commonly `sections/*.liquid`)
- **Selectors added (source of truth)**
  - For each section:
    - `data-section-test-id` value pattern used
    - Each element selector added (`data-[element-name]-test-id`, parent/child, validation, source/destination)
    - The element description (what it targets in the UI)
    - Where it was added (file + brief snippet description, e.g. “hero CTA button”)
- **Test cases mapping**
  - For each required test case (Login/HP/PLP/PDP/Cart):
    - Short description of the flow
    - Which selectors it relies on
    - Which POM method(s) implement it
    - Which spec test(s) execute it
- **How it works**
  - How the agent determined used sections from `templates/*.json`
  - How selector IDs are composed (per naming convention)
  - How to run locally + CI (`npx playwright test`)
  - Known assumptions / dependencies (e.g., required customer/account state, availability of products/freebies)
